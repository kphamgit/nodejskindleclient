...
const printHelp = () => {
    console.log(`
        ...
        print               : Print all flashcards available
        ...
        delete (card id)    : Delete a card with given id
...
const executeCommand = async (command: string, args: string[]) => {
    switch (command) {
        ...
        case "save":
            ...
            const res = await prompt("If you have recently cleared your deck, this will empty your cards file. Are you sure (y/n): ");
            deck.save(args[0]);
        ...
        case "delete":
                //const question = await prompt("Question: ");
                //const card_id = await prompt("id: ");
                //console.log("in Main deleting card: ", card_id)
                if (args.length !== 1) {
                    throw new Error();
                }
                deck.deleteFlashcard(args[0]);
            ...
            while (true) {
                ...
                try {
                    ...
                    const quality = await prompt(
                        `Recall quality (1 - ${qualityMap.length}). Smaller means less difficulty recalling: `
                    );
